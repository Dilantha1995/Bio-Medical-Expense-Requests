...
const styles = StyleSheet.create({
  ...
  advanceBox: { width: "100%", borderWidth: 1, borderColor: "#ccc", borderRadius: 2, padding: 6, marginTop: 10 },
...
export default function BillFormPDF({ doc, companyLogoBase64, companyName, timezone, currency = "MVR" }) {
  ...
  const advanceAnalysis = Number(doc.advance_received) > 0
      ...
      <Page size="A4" orientation="portrait" style={styles.page}>
        ...
        {advanceAnalysis && (
          <View style={styles.advanceBox}>
            <Text style={styles.summaryTitle}>Advance vs Spend Analysis</Text>
            <View style={styles.summaryLine}>
              <Text>Advance Taken</Text>
              <Text>{currency} {formatMVR(advanceAnalysis.advanceReceived)}</Text>
            </View>
            <View style={styles.summaryLine}>
              <Text>Spend Amount</Text>
              <Text>{currency} {formatMVR(advanceAnalysis.spendAmount)}</Text>
            </View>
            <View style={[styles.summaryLine, { fontWeight: 700 }]}>
              <Text style={{ fontWeight: 700 }}>{advanceAnalysis.isExcess ? "Excess" : "Minus"}</Text>
              <Text style={{ fontWeight: 700 }}>{currency} {formatMVR(advanceAnalysis.difference)}</Text>
            </View>
          </View>
        )}
