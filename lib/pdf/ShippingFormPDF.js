...
const styles = StyleSheet.create({
  ...
  th: { backgroundColor: "#f0f0f0", fontWeight: 700, padding: 4, borderRightWidth: 1, borderBottomWidth: 1, borderColor: "#999", fontSize: 7 },
  td: { padding: 4, borderRightWidth: 1, borderBottomWidth: 1, borderColor: "#999", fontSize: 7 },
  totalRow: { flexDirection: "row", backgroundColor: "#f5f5f5" },
  summaryBox: { width: "48%", borderWidth: 1, borderColor: "#ccc", borderRadius: 2, padding: 6, marginTop: 12 },
...
const COLS = [
  { key: "srn", label: "Srn", w: "5%" },
  { key: "date", label: "Date", w: "9%" },
  { key: "description", label: "Description", w: "18%" },
  { key: "dnNumber", label: "DN No.", w: "10%" },
  { key: "refNo", label: "Ref No.", w: "10%" },
  { key: "location", label: "Location", w: "14%" },
  { key: "expenseType", label: "Type of Expense", w: "14%" },
  { key: "amount", label: "Amount", w: "10%" },
...
function cellValue(item, key, idx) {
  switch (key) {
    ...
    case "amount": return formatMVR(shippingItemTotal(item));
...
export default function ShippingFormPDF({ doc, companyLogoBase64, companyName, timezone, currency = "MVR" }) {
  const items = doc.line_items || [];
  const byCurrency = summarizeShippingItemsByCurrency(items);
      ...
      <Page size="A4" orientation="portrait" style={styles.page}>
        ...
        <View style={styles.headerRow}>
          ...
          <View style={styles.headerCenter}>
            ...
            <Text style={styles.docTitle}>Shipping Expense Request</Text>
          </View>
        ...
        <View style={styles.table}>
          ...
          <View style={styles.totalRow}>
            <Text style={[styles.td, { width: "90%", textAlign: "right", fontWeight: 700 }]}>TOTAL</Text>
        ...
        {items.length > 0 && (
          <View style={styles.summaryBox}>
            <Text style={styles.summaryTitle}>By Currency</Text>
            {Object.entries(byCurrency).map(([k, v]) => (
        ...
        {doc.notes ? (
          <View style={{ marginTop: 8 }}>
            <Text><Text style={styles.label}>Notes: </Text>{doc.notes}</Text>
          </View>
        ) : null}
