...
export async function getAppSettings() {
  ...
  const settings = { timezone: "Indian/Maldives", currency: "MVR", signaturesEnabled: "true" };
  rows.forEach((r) => { settings[r.key] = r.value; });
  return settings;
}

/**
 * Strips the scanned signature images from a document record when the
 * "Digital Signatures" setting is turned off, so PrintableForm and the PDF
 * templates fall back to their existing "Pending"/blank-space rendering —
 * names and timestamps still show either way.
 */
export function withSignatureSetting(record, appSettings) {
  if (appSettings.signaturesEnabled !== "false") return record;
  return {
    ...record,
...
export async function getShippingRequestById(id) {
  const { rows } = await query(
    `SELECT sr.*,
        eng.full_name AS engineer_name, eng.initials AS engineer_initials, eng.designation AS engineer_designation,
        prep.full_name AS prepared_by_name, prep.signature_data AS prepared_by_signature,
     ...
     JOIN users eng ON eng.id = sr.engineer_id
     LEFT JOIN users prep ON prep.id = sr.prepared_by
     LEFT JOIN users chk ON chk.id = sr.checked_by
     LEFT JOIN users appr ON appr.id = sr.approved_by
     LEFT JOIN users del ON del.id = sr.deleted_by
     LEFT JOIN users pay ON pay.id = sr.payment_processed_by
     WHERE sr.id = $1`,
    [id]
  );
  return rows[0] || null;
