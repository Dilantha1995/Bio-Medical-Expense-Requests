...
const NO_PERMISSIONS = {
  ...
  can_manage_pm_columns: false,
  can_manage_pm_rules: false,
  can_view_reports: false,
  can_view_activity_log: false,
  can_view_engineer_performance: false,
...
export async function createSessionToken(user) {
  ...
  return new SignJWT({
    ...
    canManagePmColumns: !!rolePerms.can_manage_pm_columns,
    canManagePmRules: !!rolePerms.can_manage_pm_rules,
    canViewReports: !!rolePerms.can_view_reports,
    canViewActivityLog: !!rolePerms.can_view_activity_log,
    canViewEngineerPerformance: !!rolePerms.can_view_engineer_performance,
    canManageRoles: !!rolePerms.can_manage_roles,
  })
...
export async function requireCanManagePmColumns() {
  const session = await requireSession();
  if (!session.canManagePmColumns) {
    const err = new Error("You're not authorized to manage PM schedule columns.");
    err.status = 403;
    throw err;
  }
  return session;
...
export async function requireCanManagePmRules() {
  const session = await requireSession();
  if (!session.canManagePmRules) {
    const err = new Error("You're not authorized to manage PM schedule color rules.");
    err.status = 403;
    throw err;
  }
  return session;
...
export async function requireCanManageRoles() {
  ...
  if (!session.canManageRoles) {
    const err = new Error("You're not authorized to manage roles.");
    err.status = 403;
    throw err;
  }
  return session;
