...
export async function GET() {
  try {
    ...
    const { rows } = await query(
      `SELECT * FROM nature_of_payment_options WHERE active=true ORDER BY sort_order ASC, id ASC`
    );
...
export async function POST(req) {
  try {
    ...
    const maxOrder = await query(`SELECT COALESCE(MAX(sort_order), -1) AS m FROM nature_of_payment_options`);
    const sortOrder = maxOrder.rows[0].m + 1;

    const { rows } = await query(
      `INSERT INTO nature_of_payment_options (label, sort_order) VALUES ($1,$2) RETURNING *`,
