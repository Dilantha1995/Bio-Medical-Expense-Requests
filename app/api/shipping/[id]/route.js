...
import { getShippingRequestById as fetchShipping } from "@/lib/data";
...
export async function PATCH(req, { params }) {
  try {
    ...
    if (action === "delete") {
      ...
      await query(
        `UPDATE shipping_expense_requests SET deleted_at=now(), deleted_by=$1, deletion_reason=$2 WHERE id=$3`,
        [session.id, reason.trim(), params.id]
      );
      const updated = await fetchShipping(params.id);
      await notify(before.engineer_id, "Shipping expense request deleted", `${before.ref_number} was deleted by ${session.fullName}. Reason: ${reason.trim()}`, link);
    ...
    if (action === "start_payment" || action === "mark_payment_processed" || action === "reject_payment") {
      ...
      if (before.status !== "approved") {
        return NextResponse.json({ error: "Only approved shipping expense requests can enter payment processing." }, { status: 400 });
      }
      ...
      if (action === "start_payment") {
        await query(`UPDATE shipping_expense_requests SET payment_status='processing', payment_rejection_reason=NULL WHERE id=$1`, [params.id]);
        ...
        await query(
          `UPDATE shipping_expense_requests SET payment_status='rejected', payment_rejection_reason=$1 WHERE id=$2`,
        ...
        await query(
          `UPDATE shipping_expense_requests SET payment_status='processed', payment_slip_data=$1, payment_processed_by=$2, payment_processed_at=now() WHERE id=$3`,
    ...
    if (action === "check") {
      ...
      await query(
        `UPDATE shipping_expense_requests SET status='checked', checked_by=$1, checked_at=now() WHERE id=$2`,
        [session.id, params.id]
      );
      await notify(record.engineer_id, "Shipping expense request checked", `${record.ref_number} has been checked and is awaiting final approval.`, link);
      ...
      await query(
        `UPDATE shipping_expense_requests SET status='approved', approved_by=$1, approved_at=now() WHERE id=$2`,
        [session.id, params.id]
      );
      await notify(record.engineer_id, "Shipping expense request approved", `${record.ref_number} has been approved.`, link);
    } else if (action === "reject") {
      await query(
        `UPDATE shipping_expense_requests SET status='rejected', rejection_reason=$1 WHERE id=$2`,
        [reason || "No reason given.", params.id]
      );
      await notify(record.engineer_id, "Shipping expense request rejected", `${record.ref_number} was rejected. Reason: ${reason || "No reason given."}`, link);
    } else {
